...
int     Server::noticeHandler(const Message &message, Client *client)
{
	Client                  *sender;
	Client                  *targetClient;
	Channel                 *targetChannel;
	...
	else
	{
		...
		if (message.getParameter(0).at(0) == '#' || message.getParameter(0).at(0) == '&')
		{
			...
			joinedUsers = targetChannel->getUsersList(this->serverName);
			for (int i = 0; i < (int)joinedUsers.size(); i++)
				this->sendMessage(Message(":" + getClientPrefix(sender), "NOTICE", getTextMessage(message)), joinedUsers[i]);

			// 다른 서버에 메시지 보냄
			// :seunkim PRIVMSG #my :hello
			this->broadcastMessage(Message(":" + sender->getInfo(NICK), "NOTICE", getTextMessage(message)), client);
		}
		else
		{
			if ((this->clientList.size() != 0) && (this->clientList.find(message.getParameter(0)) != this->clientList.end()))
			{
				targetClient = this->clientList[message.getParameter(0)];
				if (message.getPrefix().empty())
					this->sendMessage(Message(":" + getClientPrefix(client), "NOTICE", getTextMessage(message)), targetClient);
				else
					this->sendMessage(Message(message.getPrefix(), "NOTICE", getTextMessage(message)), targetClient);
			}
			else if (this->sendClients.find(message.getParameter(0)) != this->sendClients.end())
			{
				targetClient = &this->sendClients[message.getParameter(0)];
